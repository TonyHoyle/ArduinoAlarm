...
// Adapted for nonblocking November 2016 by Tony Hoyle, tony@hoyle.me.uk
...
int DhcpClass::beginWithDHCP(uint8_t *mac, const char *hostName, unsigned long responseTimeout)
{
  _dhcpLeaseTime = 0;
  _dhcpT1 = 0;
  _dhcpT2 = 0;
...
//return:0 on error, 1 if request is sent
...
int DhcpClass::transact_DHCP_lease()
{
  ...
  if (_dhcp_state != STATE_DHCP_START && _dhcpUdpSocket.parsePacket() <= 0)
  {
    if(millis() > (_lastDhcpMessage + _responseTimeout)) /* No response, restart DHCP transaction */
      _dhcp_state = STATE_DHCP_START;
    else
      return result;
  }
...
#if DEBUG
  Serial.print("transact_dhcp_lease() state = ");
...
#if DEBUG
  Serial.println("send_DHCP_MESSAGE()");
#endif

  if (-1 == _dhcpUdpSocket.beginPacket(dest_addr, DHCP_SERVER_PORT))
  {
#if DEBUG
	Serial.println("Udp Socket beginPacket failed");
#endif
    // FIXME Need to return errors
    return;
  }

  _lastDhcpMessage = millis();
